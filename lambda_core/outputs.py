...
def render_markdown(contract: dict[str, Any], *, job_source: str, profile_source: str) -> str:
    ...
    family = contract.get("role_family") or {}
    ...
    family_block = (
        f"**Primary family:** `{family.get('primary_family', '')}`\n\n"
        f"**Why this family:** {family.get('why_this_family', '')}\n\n"
        f"**Secondary families**\n{_bullets(family.get('secondary_families') or [])}\n\n"
        f"**Excluded families**\n{_bullets(family.get('excluded_families') or [])}"
    )
    ...
    signature = _bullets(role.get("role_signature_claims") or [])
    ...
    metrics = "\n\n".join(
        ...
        for item in contract.get("operational_metrics_contract") or []
    )

    bounds = contract.get("automation_boundaries") or {}
    boundaries_block = (
        f"### Safe to automate\n{_bullets(bounds.get('safe_to_automate') or [])}\n\n"
        f"### Requires human escalation\n{_bullets(bounds.get('requires_human_escalation') or [])}\n\n"
        f"### Fail-closed conditions\n{_bullets(bounds.get('fail_closed_conditions') or [])}\n\n"
        f"### Manual approval gates\n{_bullets(bounds.get('manual_approval_gates') or [])}"
    )

    cap_proof = contract.get("capstone_proof_contract") or {}
    capstone_block = (
        f"### What it must demonstrate\n{_bullets(cap_proof.get('what_it_must_demonstrate') or [])}\n\n"
        f"### Required failure injections\n{_bullets(cap_proof.get('required_failure_injections') or [])}\n\n"
        f"### Required measurements\n{_bullets(cap_proof.get('required_measurements') or [])}\n\n"
        f"### Required docs\n{_bullets(cap_proof.get('required_docs') or [])}\n\n"
        f"### Hiring-manager readout\n\n{cap_proof.get('hiring_manager_readout', '')}"
    )
...
## 14. Operational metrics contract
...
## 16. Capstone proof contract

{capstone_block}

## 17. Interview readiness map
...
- **Avoids blind full-automation language:** {_yes_no(guards.get('avoids_blind_full_automation_language', False))}
- **Has ops metrics + automation boundaries:** {_yes_no(guards.get('has_operational_metrics_and_automation_boundaries', False))}
